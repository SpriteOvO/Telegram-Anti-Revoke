...
[[noreturn]] void DoError(const std::string &Content, bool bReport)
{
    ...
    do {
        Result = MessageBoxA(
            nullptr, Message.c_str(), "Anti-Revoke Plugin", MB_ABORTRETRYIGNORE | MB_ICONERROR);
        // lol..
    ...
    if (bReport) {
        ...
        // Using ShellExecute will cause freezing. I guess it may be caused by different Runtime
        // libraries like cross-module malloc/free. So replace ShellExecute with system, there is
        // not much difference anyway.

        // ShellExecute(nullptr, L"open",
        // L"https://github.com/SpriteOvO/Telegram-Anti-Revoke/issues", nullptr, nullptr,
        // SW_SHOWNORMAL);
...
void Initialize()
{
    auto CustomLogger = std::make_shared<spdlog::logger>(
        "Main",
        std::initializer_list<spdlog::sink_ptr>{std::make_shared<CCustomFileSink<>>("ArLog.txt")});
    ...
    spdlog::set_error_handler(
        [](const std::string &Message) { DoError("Spdlog error.\n" + Message, true); });
}
